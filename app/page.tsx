  ...
  return (
    <main className={`${darkMode ? "bg-gray-900 text-white" : "bg-yellow-50 text-gray-900"} relative min-h-screen flex flex-col items-center font-sans p-6 transition-colors duration-500 overflow-hidden`}>
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div className={`w-[200%] h-[200%] rounded-full bg-gradient-to-tr from-pink-500 via-yellow-400 to-green-400 opacity-20 animate-bgWave ${darkMode ? "" : "opacity-10"}`} style={{ transform: `translateY(${scrollY * 0.02}px)` }}></div>
        <div className={`w-[150%] h-[150%] rounded-full bg-gradient-to-br from-blue-400 via-purple-500 to-pink-400 opacity-20 animate-bgWave2 ${darkMode ? "" : "opacity-10"}`} style={{ transform: `translateY(${scrollY * -0.015}px)` }}></div>
      </div>
      ...
      <div className="w-full flex justify-end mb-4 relative z-10">
        <button
          ...
          className="px-4 py-2 rounded-full font-semibold focus:outline-none shadow-md transition-colors duration-300"
      ...
      >
        ...
        <p className={`text-lg md:text-xl mb-6 animate-fadeIn delay-100 ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
          Scan the QR below to order on WhatsApp instantly
        </p>

        <div className="inline-block animate-pulse mb-2 transform transition-transform duration-500 hover:scale-110 hover:shadow-2xl" style={{ transform: `translateY(${scrollY * 0.08}px)` }}>
          <Image 
        ...
        <p className={`text-lg mt-2 animate-fadeIn delay-200 ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
          Or click to WhatsApp:{" "}
          <a 
            href="https://wa.me/2347062319517" 
            className={`font-semibold hover:underline ${darkMode ? "text-green-400" : "text-green-600"}`}
            target="_blank"
      ...
      <section className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 relative z-10">
        {products.map((product, idx) => (
          <div 
            ...
            className={`relative p-6 flex flex-col items-center text-center transform transition-transform duration-300 hover:scale-105 hover:shadow-2xl rounded-2xl shadow-lg ${
              loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
            } ${darkMode ? "bg-gray-800" : "bg-white"}`}
            style={{ transitionDelay: `${idx * 200}ms`, transform: `translateY(${scrollY * 0.03}px)` }}
          >
            <div className="absolute -top-4 -left-4 w-32 h-32 rounded-full opacity-20 bg-gradient-to-tr from-pink-400 to-yellow-400 animate-float z-0"></div>
            <div className="absolute -bottom-4 -right-4 w-24 h-24 rounded-full opacity-20 bg-gradient-to-br from-green-400 to-blue-400 animate-float z-0"></div>
            ...
            <Image 
              ...
              width={280} 
              height={280} 
              className="object-cover mb-4 rounded-xl shadow-md relative z-10"
            />
      ...
      <style jsx>{`
        ...
        @keyframes bgWave {
          ...
          50% { transform: translateX(20%) translateY(5%) rotate(10deg); }
        ...
        @keyframes bgWave2 {
          ...
          50% { transform: translateX(-15%) translateY(10%) rotate(-10deg); }
